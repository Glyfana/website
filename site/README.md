...
- `Showcase`: real screenshot gallery when `screenshots-manifest.json` has items, or the built-in mockup when it is empty
...
- `../scripts/capture-glyfana-screenshots.cjs`
  - launches the local Glyfana Electron app and exports `editor-main.png`, `editor-image.png`, and `editor-source.png`
  - uses `capturePage()` so the screenshots come from the real renderer, not a browser mockup
...
- The checked-in screenshot set was generated by `scripts/capture-glyfana-screenshots.cjs` against the local Glyfana app build.
